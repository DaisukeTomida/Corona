...
class Logger {
    ...
    private function out($level, $msg) {
        if(Config::IS_LOGFILE) {
            ...
            $logFilePath = $_SERVER['DOCUMENT_ROOT'] . '/corona/' . Config::LOGDIR_PATH . Config::LOGFILE_NAME . '.log';
            ...
            if(Config::LOGFILE_MAXSIZE < filesize($logFilePath)) {
                ...
                $oldPath = $_SERVER['DOCUMENT_ROOT'] . '/corona/' . Config::LOGDIR_PATH . Config::LOGFILE_NAME . '_' . date('YmdHis');
                ...
                if ($dh = opendir($_SERVER['DOCUMENT_ROOT'] . '/corona/' . Config::LOGDIR_PATH)) {
                    while (($fileName = readdir($dh)) !== false) {
                        ...
                        if($pm === 1) {
                            ...
                            if($logCreatedDate < $retentionDate) {
                                unlink($_SERVER['DOCUMENT_ROOT'] . '/corona/' . Config::LOGDIR_PATH . '/' . $fileName);
                            }
                        }
                    }
                    closedir($dh);
